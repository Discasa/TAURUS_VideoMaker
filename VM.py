...
CONFIG_JSON_PATH = SCRIPT_DIR / "video_creator_config.json"

FFMPEG = FFMPEG_BIN / "ffmpeg.exe"
FFPROBE = FFMPEG_BIN / "ffprobe.exe"

EXTENSOES_AUDIO = [".mp3", ".wav", ".m4a", ".aac", ".flac", ".ogg", ".opus", ".wma"]
EXTENSOES_VIDEO = [".mp4", ".mov", ".mkv", ".avi", ".webm", ".gif"]
...
class RenderCancelado(Exception): pass
class ErroRender(Exception): pass
...
class ControleExecucao:
    ...
    def definir_processo(self, processo):
        with self.lock: self.processo_atual = processo
    ...
    def excluir_arquivos_cancelados(self):
        with self.lock: arquivos = list(self.arquivos_para_excluir)
    ...
    def _encerrar_arvore_processo(self, processo):
        try:
            if os.name == "nt":
                subprocess.run(["taskkill", "/PID", str(processo.pid), "/T", "/F"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess, "CREATE_NO_WINDOW") else 0)
            else:
                try: os.killpg(os.getpgid(processo.pid), signal.SIGTERM)
                except: processo.terminate()
        except:
            try: processo.kill()
            except: pass
...
def caminho_pasta_fontes_windows() -> Path:
    if os.name == "nt": return Path(os.environ.get("WINDIR", r"C:/Windows")) / "Fonts"
...
def limpar_titulo_musica(arquivo: Path) -> str:
    ...
    return re.sub(r"\s+", " ", titulo).strip() or arquivo.stem
...
def escape_drawtext(texto: str) -> str:
    return texto.replace("\\", "\\\\").replace(":", "\\:").replace("'", "\\'").replace(",", "\\,").replace("[", "\\[").replace("]", "\\]").replace("%", "\\%").replace("\n", " ")
...
def limpar_hex(cor: str, fallback: str = "#FFFFFF") -> str:
    ...
    if re.fullmatch(r"#[0-9A-Fa-f]{6}", cor): return cor.upper()
    return fallback
...
def opcoes_posicao():
    return [
        ("Inferior direita", "inferior_direita"), ("Inferior esquerda", "inferior_esquerda"), ("Inferior centro", "inferior_centro"),
        ("Superior direita", "superior_direita"), ("Superior esquerda", "superior_esquerda"), ("Superior centro", "superior_centro"),
        ("Centro", "centro"),
    ]

def popular_combo_posicoes(combo, valor_atual: str):
    for texto, valor in opcoes_posicao(): combo.addItem(texto, valor)
    idx = combo.findData(valor_atual)
    combo.setCurrentIndex(max(0, idx))

def ass_alignment_from_position(position: str) -> int:
    return {"inferior_esquerda": 1, "inferior_direita": 3, "superior_esquerda": 7, "superior_direita": 9, "superior_centro": 8, "inferior_centro": 2, "centro": 5}.get(position, 1)

def overlay_position_expr(position: str, margin_x: int, margin_y: int) -> tuple[str, str]:
    posicoes = {
        "inferior_direita": (f"W-w-{margin_x}", f"H-h-{margin_y}"), "inferior_esquerda": (f"{margin_x}", f"H-h-{margin_y}"),
        "inferior_centro": ("(W-w)/2", f"H-h-{margin_y}"), "superior_direita": (f"W-w-{margin_x}", f"{margin_y}"),
        "superior_esquerda": (f"{margin_x}", f"{margin_y}"), "superior_centro": ("(W-w)/2", f"{margin_y}"), "centro": ("(W-w)/2", "(H-h)/2"),
    }
...
def gerar_pasta_saida_padrao() -> Path:
    return SCRIPT_DIR / f"render_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"


# ==========================
# MOTOR DE RENDERIZAÇÃO
# ==========================
# A UI nova reutiliza o backend estável do LoFi_VideoMaker.py para manter
# FFmpeg, cancelamento, pausa, logs e empacotamento em um único lugar.

def carregar_backend_principal():
    caminho_backend = SCRIPT_DIR / "LoFi_VideoMaker.py"
    if not caminho_backend.exists():
        raise RuntimeError(f"Backend principal não encontrado: {caminho_backend}")

    spec = importlib.util.spec_from_file_location("lofi_videomaker_backend", caminho_backend)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Não foi possível carregar o backend principal: {caminho_backend}")

    modulo = importlib.util.module_from_spec(spec)
    sys.modules.setdefault(spec.name, modulo)
    spec.loader.exec_module(modulo)
    return modulo

BACKEND_PRINCIPAL = carregar_backend_principal()
RenderEngine = BACKEND_PRINCIPAL.RenderEngine
WorkerRender = BACKEND_PRINCIPAL.WorkerRender
...
try:
    ...
    from PySide6.QtGui import QColor, QCursor, QFont, QFontDatabase, QPainter, QPen, QPixmap, QTextCursor, QIcon
    from PySide6.QtWidgets import (
        QApplication, QAbstractSpinBox, QCheckBox, QColorDialog, QComboBox, QDialog, QDoubleSpinBox,
